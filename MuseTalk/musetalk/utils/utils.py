# In L:\1 Things\Code11\LiveAct\MuseTalk\musetalk\utils\utils.py
...
from MuseTalk.musetalk.musetalk_models.vae import VAE
from MuseTalk.musetalk.musetalk_models.unet import UNet,PositionalEncoding
# --- End imports ---


# --- Calculate MUSE_TALK_PROJECT_ROOT_FROM_UTILS relative to this utils.py file ---
# This utils.py file is at: L:\...\MuseTalk\musetalk\utils\utils.py
UTILS_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
# MUSE_TALK_PACKAGE_ROOT will be: L:\...\MuseTalk\musetalk\
MUSE_TALK_PACKAGE_ROOT = os.path.dirname(UTILS_SCRIPT_DIR)
# MUSE_TALK_PROJECT_ROOT_FROM_UTILS will be: L:\...\MuseTalk\  (This is the root of the MuseTalk project)
MUSE_TALK_PROJECT_ROOT_FROM_UTILS = os.path.dirname(MUSE_TALK_PACKAGE_ROOT)
...
def load_all_model(
    ...
    device=None,
    # Add a parameter for use_float16
    use_float16=False # <-- NEW PARAMETER
):
    # --- Construct ABSOLUTE paths using MUSE_TALK_PROJECT_ROOT_FROM_UTILS ---
    # This makes paths independent of CWD when load_all_model is called.

    # For unet_model_path:
    # If the path passed in (e.g., from args_config) is already absolute, use it.
    # Otherwise, assume it's relative to the MuseTalk project root.
    if not os.path.isabs(unet_model_path):
        abs_unet_model_path = os.path.normpath(os.path.join(MUSE_TALK_PROJECT_ROOT_FROM_UTILS, unet_model_path))
    else:
        abs_unet_model_path = os.path.normpath(unet_model_path)

    # For unet_config:
    if not os.path.isabs(unet_config):
        abs_unet_config = os.path.normpath(os.path.join(MUSE_TALK_PROJECT_ROOT_FROM_UTILS, unet_config))
    else:
        abs_unet_config = os.path.normpath(unet_config)
    
    # For VAE: vae_type is just the sub-directory name like "sd-vae" or "sd-vae-ft-mse"
    # You confirmed your VAE is in "models/sd-vae/"
    abs_vae_model_path = os.path.normpath(os.path.join(MUSE_TALK_PROJECT_ROOT_FROM_UTILS, "models", vae_type))

    print(f"[utils.py] VAE path: {abs_vae_model_path}")
    print(f"[utils.py] UNet model path: {abs_unet_model_path}")
    print(f"[utils.py] UNet config path: {abs_unet_config}")

    # Ensure the constructed paths exist (optional but good for debugging)
    if not os.path.isdir(abs_vae_model_path) or not os.path.exists(os.path.join(abs_vae_model_path, "config.json")):
        raise FileNotFoundError(f"VAE directory or config.json not found at calculated absolute path: {abs_vae_model_path}")
    if not os.path.isfile(abs_unet_model_path):
        raise FileNotFoundError(f"UNet model file not found at calculated absolute path: {abs_unet_model_path}")
    if not os.path.isfile(abs_unet_config):
        raise FileNotFoundError(f"UNet config file not found at calculated absolute path: {abs_unet_config}")
    # --- End ABSOLUTE path construction ---

    # Pass use_float16 to the VAE constructor
    vae = VAE(
        model_path = abs_vae_model_path,
        use_float16=use_float16 # <-- PASS THIS PARAMETER
    )
    
    unet = UNet( # UNet class is in musetalk.models.unet
        unet_config=abs_unet_config,     # Pass the ABSOLUTE path
        model_path=abs_unet_model_path,  # Pass the ABSOLUTE path
        device=device
    )
    pe = PositionalEncoding(d_model=384) # PositionalEncoding also in musetalk.models.unet
    return vae, unet, pe
